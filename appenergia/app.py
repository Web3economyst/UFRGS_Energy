...
if not df_raw.empty:
    ...
    with tab1:
        ...
        if not df_ocupacao.empty:
            ...
            st.divider()
            st.markdown("#### 🔍 Detalhe da Ocupação (Foco < 135 pessoas)")
            st.caption("Visualização focada no dia a dia, limitando o eixo Y para facilitar a leitura.")
            ...
            fig_zoom.add_hline(y=135, line_dash="dash", line_color="red", annotation_text="Limite Visualização (135)")
